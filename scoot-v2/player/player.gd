...
@export var jump_initial_impulse := 14.0
...
func _physics_process(delta: float) -> void:
	...
	# Camera toggle (V key or Triangle)
	if Input.is_action_just_pressed("camera_toggle"):
		_camera_controller.toggle_camera_mode()

	# Trick input — only while airborne
	if not is_on_floor():
		_trick_system.handle_trick_input()

	# Landing — bank the combo
	if is_just_on_floor:
		_trick_system.on_landed()
		_landing_sound.play()
